<?php

namespace App\Http\Resources\Admin;

use Illuminate\Http\Resources\Json\JsonResource;

class BecomeSponsorSettingDetailResource extends JsonResource
{
    public function toArray($request)
    {
        
        return [
            'id' => $this->id,
            'become_sponsor_setting_id' => $this->become_sponsor_setting_id,
            'language_id' => $this->language_id,
            'name_label' => $this->name_label,
            'name_error' => $this->name_error,
            'name_placeholder' => $this->name_placeholder,
            'company_name_label' => $this->company_name_label,
            'company_name_error' => $this->company_name_error,
            'company_name_placeholder' => $this->company_name_placeholder,
            'email_label' => $this->email_label,
            'email_error' => $this->email_error,
            'email_placeholder' => $this->email_placeholder,
            'contact_number_label' => $this->contact_number_label,
            'contact_number_error' => $this->contact_number_error,
            'contact_number_placeholder' => $this->contact_number_placeholder,
            'message_label' => $this->message_label,
            'message_error' => $this->message_error,
            'message_placeholder' => $this->message_placeholder,
            'image_label' => $this->image_label,
            'image_placeholder' => $this->image_placeholder,
            'image_error' => $this->image_error,
            'url_label' => $this->url_label,
            'url_error' => $this->url_error,
            'url_placeholder' => $this->url_placeholder,
            'time_to_call_label' => $this->time_to_call_label,
            'time_to_call_error' => $this->time_to_call_error,
            'time_to_call_am_label' => $this->time_to_call_am_label,
            'time_to_call_pm_label' => $this->time_to_call_pm_label,
            'appointment_label' => $this->appointment_label,
            'appointment_error' => $this->appointment_error,
            'appointment_yes_option' => $this->appointment_yes_option,
            'appointment_no_option' => $this->appointment_no_option,
            'appointment_date_label' => $this->appointment_date_label,
            'appointment_date_error' => $this->appointment_date_error,
            'feature_image_label' => $this->feature_image_label,
            'feature_image_error' => $this->feature_image_error,
            'feature_image_placeholder' => $this->feature_image_placeholder,
            'summary_label' => $this->summary_label,
            'summary_error' => $this->summary_error,
            'summary_placeholder' => $this->summary_placeholder,
            'detail_description_label' => $this->detail_description_label,
            'detail_description_error' => $this->detail_description_error,
            'detail_description_placeholder' => $this->detail_description_placeholder,
            'submit_btn_text' => $this->submit_btn_text,
            'sponsorship_section_heading' => $this->sponsorship_section_heading,
            'enter_amount_placeholder' => $this->enter_amount_placeholder,
            'talk_to_us_first_label' => $this->talk_to_us_first_label,
            'talk_to_us_first_description' => $this->talk_to_us_first_description,
            'no_amounts_message' => $this->no_amounts_message,
            'contact_preferences_heading' => $this->contact_preferences_heading,
            'best_time_to_call_label' => $this->best_time_to_call_label,
            'preferred_date_label' => $this->preferred_date_label,
            'call_time_morning' => $this->call_time_morning,
            'call_time_afternoon' => $this->call_time_afternoon,
            'call_time_evening' => $this->call_time_evening,
            'account_details_heading' => $this->account_details_heading,
            'contact_name_placeholder' => $this->contact_name_placeholder,
            'email_hint' => $this->email_hint,
            'password_label' => $this->password_label,
            'password_hint' => $this->password_hint,
            'confirm_password_label' => $this->confirm_password_label,
            'optional_text' => $this->optional_text,
            'brand_story_heading' => $this->brand_story_heading,
            'featured_image_hint' => $this->featured_image_hint,
            'logo_hint' => $this->logo_hint,
            'summary_placeholder_long' => $this->summary_placeholder_long,
            'detail_description_placeholder_long' => $this->detail_description_placeholder_long,
            'message_placeholder_long' => $this->message_placeholder_long,
            'featured_image_idle' => $this->featured_image_idle,
            'logo_idle' => $this->logo_idle,
            'payment_method_heading' => $this->payment_method_heading,
            'debit_credit_label' => $this->debit_credit_label,
            'cardholder_name_label' => $this->cardholder_name_label,
            'terms_privacy_label' => $this->terms_privacy_label,
            'terms_privacy_label_1' => $this->terms_privacy_label_1,
            'terms_privacy_label_2' => $this->terms_privacy_label_2,
            'terms_privacy_label_3' => $this->terms_privacy_label_3,
            'terms_privacy_label_4' => $this->terms_privacy_label_4,
            'terms_privacy_label_5' => $this->terms_privacy_label_5,
            'donation_non_refundable_label' => $this->donation_non_refundable_label,
            'processing_text' => $this->processing_text,
            'reactivate_btn_text' => $this->reactivate_btn_text,
            'become_sponsor_btn_text' => $this->become_sponsor_btn_text,
            'become_sponsor_setting' => new BecomeSponsorSettingResource($this->whenLoaded('becomeSponsorSetting')),
            // SponsorshipsList-specific labels
            'manage_sponsorship_heading' => $this->manage_sponsorship_heading,
            'manage_sponsorship_subtitle' => $this->manage_sponsorship_subtitle,
            'manage_sponsorship_thanks' => $this->manage_sponsorship_thanks,
            'add_another_sponsorship_btn' => $this->add_another_sponsorship_btn,
            'loading_sponsorships' => $this->loading_sponsorships,
            'no_sponsorships_heading' => $this->no_sponsorships_heading,
            'no_sponsorships_message' => $this->no_sponsorships_message,
            'create_first_sponsorship_btn' => $this->create_first_sponsorship_btn,
            'status_active' => $this->status_active,
            'status_pending' => $this->status_pending,
            'status_inactive' => $this->status_inactive,
            'change_frequency_btn' => $this->change_frequency_btn,
            'collapse_btn' => $this->collapse_btn,
            'payment_status_paid' => $this->payment_status_paid,
            'payment_status_pending' => $this->payment_status_pending,
            'payment_status_not_required' => $this->payment_status_not_required,
            'payment_status_failed' => $this->payment_status_failed,
            'payment_status_refunded' => $this->payment_status_refunded,
            'label_amount' => $this->label_amount,
            'label_beneficiary' => $this->label_beneficiary,
            'label_created' => $this->label_created,
            'label_payment_method' => $this->label_payment_method,
            'edit_btn' => $this->edit_btn,
            'reactivation_panel_message' => $this->reactivation_panel_message,
            'next_billing_date_label' => $this->next_billing_date_label,
            'upgrade_btn' => $this->upgrade_btn,
            'reactivate_heading' => $this->reactivate_heading,
            'loading_overlay_text' => $this->loading_overlay_text,
            'payment_method_ending_in' => $this->payment_method_ending_in,
        ];
    }
}
