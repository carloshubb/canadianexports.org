            ...
            if (!empty($general_setting['admin_email'])) {
                ...
                if (!empty($adminEmailsArr)) {
                    if (count($adminEmailsArr) > 1) {
                        ...
                        Mail::to($to_email)->cc(array_values($adminEmailsArr))->send(new CoffeeWallAdminNotificationMail($coffeeWallet));
                    } else {
                        Mail::to($adminEmailsArr[0])->send(new CoffeeWallAdminNotificationMail($coffeeWallet));
                    }
                    Log::info('Coffee on Wall admin notification email sent (PayPal)');
                }
            }
        } catch (\Exception $e) {
            Log::error('Failed to send Coffee on Wall admin notification email (PayPal)', ['error' => $e->getMessage()]);
        }
