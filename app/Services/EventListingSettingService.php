...
class EventListingSettingService
{
    public function validation($languages, $validationRule, $errorMessages)
    {
        foreach ($languages as $language) {

            $validationRule = array_merge($validationRule, ['no_event_found_text.no_event_found_text_' . $language->id => ['nullable', 'string']]);
            $errorMessages = array_merge($errorMessages, ['no_event_found_text.no_event_found_text_' . $language->id . '.required' => 'No event found text in ' . $language->name . ' is required.']);

            $validationRule = array_merge($validationRule, ['user_has_events_text.user_has_events_text_' . $language->id => ['nullable', 'string']]);
            $errorMessages = array_merge($errorMessages, ['user_has_events_text.user_has_events_text_' . $language->id . '.required' => 'User has events text in ' . $language->name . ' is required.']);

            $validationRule = array_merge($validationRule, ['add_event_btn_text.add_event_btn_text_' . $language->id => ['nullable', 'string']]);
            $errorMessages = array_merge($errorMessages, ['add_event_btn_text.add_event_btn_text_' . $language->id . '.required' => 'Add event button text in ' . $language->name . ' is required.']);
            ...
            $validationRule = array_merge($validationRule, ['events_text.events_text_' . $language->id => ['nullable', 'string']]);
            $errorMessages = array_merge($errorMessages, ['events_text.events_text_' . $language->id . '.required' => 'Events text in ' . $language->name . ' is required.']);
    ...
    public function fields($eventListingSetting, $language, $request)
    {
        return [
            'event_listing_setting_id' => $eventListingSetting->id,
            'language_id' => $language->id,
            'no_event_found_text' => isset($request['no_event_found_text']['no_event_found_text_' . $language->id]) ? $request['no_event_found_text']['no_event_found_text_' . $language->id] : null,
            'user_has_events_text' => isset($request['user_has_events_text']['user_has_events_text_' . $language->id]) ? $request['user_has_events_text']['user_has_events_text_' . $language->id] : null,
            'add_event_btn_text' => isset($request['add_event_btn_text']['add_event_btn_text_' . $language->id]) ? $request['add_event_btn_text']['add_event_btn_text_' . $language->id] : null,
            ...
            'events_text' => isset($request['events_text']['events_text_' . $language->id]) ? $request['events_text']['events_text_' . $language->id] : null,
    ...
    public function store($eventListingSetting, $language, $request)
    {
        $fields = $this->fields($eventListingSetting, $language, $request);
        EventListingSettingDetail::create($fields);
        return true;
    }

    public function update($eventListingSetting, $language, $request)
    {
        $fields = $this->fields($eventListingSetting, $language, $request);
        EventListingSettingDetail::whereEventListingSettingId($eventListingSetting->id)->whereLanguageId($language->id)->update($fields);
        return true;
    }
}
