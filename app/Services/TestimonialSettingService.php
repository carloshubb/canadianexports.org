...
class TestimonialSettingService
{
    ...
    public function fields($testimonialSetting, $language, $request)
    {
        return [
            'testimonial_setting_id' => $testimonialSetting->id,
    ...
    public function store($testimonialSetting, $language, $request)
    {
        $fields = $this->fields($testimonialSetting, $language, $request);
        TestimonialSettingDetail::create($fields);
        return true;
    }

    public function update($testimonialSetting, $language, $request)
    {
        $fields = $this->fields($testimonialSetting, $language, $request);
        TestimonialSettingDetail::whereTestimonialSettingId($testimonialSetting->id)->whereLanguageId($language->id)->update($fields);
        return true;
    }
}
