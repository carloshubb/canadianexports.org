            ...
            <div class="flex min-h-full flex-col justify-center py-4 md:py-12 px-4 sm:px-6 lg:px-8">
                <div class="sm:mx-auto sm:w-full sm:max-w-2xl">
                    <h3 class="can-exp-h2 text-primary text-center">
                        {{-- Unsubscribe from info letter email --}}
                ...
                <div class="mt-2 sm:mx-auto sm:w-full sm:max-w-2xl">
                    <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                        {{-- <h3 class="can-exp-h2 text-primary text-center"> --}}
                            {{-- Don’t miss out --}}
                            {{-- {{isset($unsubscribe_email_setting['header_subheading_holiday']) ? $unsubscribe_email_setting['header_subheading_infoletter'] : ''}}
                        </h3>
                        <hr /> --}}
                        <!-- Session Status -->
                        @if (session('status'))
                        ...
                        @endif
                        <form class="space-y-6" method="POST" action="{{ route('front.unsubscribe-info-letter') }}">
                            @csrf
                            ...
                            {{-- <p>
                                ...
                                information from our Info Letter?
                            </p>
                            <p>

                                By clicking Unsubscribe, you’ll be removed from all our general email communications
                            </p> --}}
                            <p>{{isset($unsubscribe_email_setting['paragraph_text']) ? $unsubscribe_email_setting['paragraph_text'] : ''}}</p>
                            {{-- <p>
                            ...
                            {{-- <hr /> --}}
                            {{-- <p>This will not unsubscribe you from info letter related emails</p> --}}
                            {{-- <p>{{isset($unsubscribe_email_setting['footer_info_text']) ? $unsubscribe_email_setting['footer_info_text'] : ''}}</p> --}}
                        </form>
                    </div>
                </div>
            </div>
