<template>
    <div>
        <div class="overflow-auto">
            <table
                class="w-full bg-white border border-collapsed overflow-auto"
                v-if="inquiries && !is_home_page"
            >
                <thead>
                    <tr class="text-center">
                        <th
                            class="px-4 py-3 text-primary text-base md:text-base lg:text-lg font-FuturaMdCnBT font-normal"
                        >
                            {{
                                JSON.parse(i2b_setting)?.[
                                    "i2_b_setting_detail"
                                ]?.[0]?.["title_heading"] || ""
                            }}
                        </th>
                ...
                <tbody>
                    <tr
                        ...
                        :id="`i2b-${inquiry.id}`"
                    >
                        <td class="px-4 py-3 text-base md:text-base lg:text-lg">
                            <a
                        ...
                        <td
                            class="px-4 py-3 text-center text-base md:text-base lg:text-lg"
                        >
                        ...
                        >
                            ${{
                                toFormattedNumber(inquiry?.["estimated_value"])
                            }}
                        </td>
                        <td
                            class="px-4 py-3 text-center text-base md:text-base lg:text-lg"
                        ...
                        >
                            ...
                            >
                                <img
                                    class="h-6 w-6 cursor-pointer md:mt-2"
                        ...
                        <div class="bg-white p-4">
                            <div class="sm:items-start">
                                <div class="sm:mt-0">
                                    ...
                                    >
                                        <h3
                                            class="card-heading text-primary"
                                            id="modal-title"
                                        >
                                            {{
    ...
    type="submit"
    class="button-exp-fill"
    @click.prevent="handlePaidPackageButtonClick"
    v-if="
      user &&
      (JSON.parse(user)?.registration_package?.package_type == 'featured' ||
        JSON.parse(user)?.registration_package?.package_type == 'premium')
    "
  >
    {{ getRegistrationTypeText() }}
  </button>
                                        ...
                                        <form @submit.prevent="loginCustomer()">
                                            ...
                                            >
                                                <a
                                                    aria-label="Candian Exporters"
                                                    :href="register_url"
                                                    class="button-exp-no-fill px-10"
                                                    >{{
                                                        JSON.parse(
                                                            modal_setting
                                                        )
    ...
    methods: {
      ...
      if (user && !user.is_package_amount_paid) {
        ...
        loginCustomer() {
            ...
            axios
                .post(
                    `${process.env.MIX_APP_URL}/logged-in-user`,
                    this.login_form
                )
                .then((res) => {
                    this.loading = 0;
