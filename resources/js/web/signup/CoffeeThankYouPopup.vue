<template>
    <div v-if="isVisible" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <!-- Background overlay -->
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" @click="closePopup"></div>
                        ...
                        <div class="mt-3 text-center sm:mt-0 w-full">
                            ...
                            <div class="mt-4">
                                <p class="text-lg text-gray-700">
                                    This Coffee comes from <span class="font-semibold text-primary">{{ displayDonorName }}</span>, who chose to support Canadian small businesses like yours.
                                </p>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    computed: {
        displayDonorName() {
            return this.donorName || 'A Kind Supporter';
        }
    },
    methods: {
        closePopup() {
...
.text-primary {
    color: #1e40af; /* Adjust to match your primary color */
}

.from-xblue {
    /* Adjust to match your xblue color */
}
</style>