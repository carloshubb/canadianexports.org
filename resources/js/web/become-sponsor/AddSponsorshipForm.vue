<template>
  <div class="bg-white rounded-lg shadow-lg p-6 md:p-8">
    <div class="mb-6">
      <h1 class="text-3xl font-bold text-primary mb-2">{{ __("Add New Sponsorship") }}</h1>
      <p class="text-gray-600">{{ __("Create an additional sponsorship under your account") }}</p>
    </div>
    ...
    <form @submit.prevent="processPayment()">
      ...
      <div class="bg-white rounded-lg overflow-hidden shadow-3xl my-6">
        <div class="px-4 py-3 sm:px-6 text-left bg-gradient-to-r from-primary via-primary to-secondary rounded-t-md">
          <h4 class="text-white">{{ __("Choose Your Option") }}</h4>
        </div>
        <div class="p-6">
          <fieldset class="flex flex-col md:flex-row items-stretch w-full overflow-hidden rounded-md shadow-sm gap-4">
            ...
            <div class="flex-1">
              <label
                ...
                <span class="font-bold">{{ __("Enter Your Amount") }}</span>
                <span class="text-sm mt-2 opacity-90">{{ __("Make a sponsorship payment now") }}</span>
              </label>
            </div>
            ...
            <div class="flex-1">
              <label
                ...
                <input type="radio" name="sponsorship_option" :value="true" class="sr-only"
                  v-model="form.talk_to_us_first" />
                <span class="text-2xl mb-2">📞</span>
                <span class="font-bold">{{ __("Talk to Us First") }}</span>
                <span class="text-sm mt-2 opacity-90">{{ __("Let's discuss before you commit") }}</span>
              </label>
            </div>
          </fieldset>
        </div>
      ...
      <div v-if="!form.talk_to_us_first" class="bg-white rounded-lg overflow-hidden shadow-3xl my-6">
        ...
        <div class="p-6">
          ...
          <div class="relative w-full" id="beneficiary_ids">
            <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg font-medium">
              {{ __("Select Beneficiary") }} <span class="text-red-500">*</span>
            </label>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="relative w-full">
              <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg">
                {{ __("Preferred Call Time") }} <span class="text-red-500">*</span>
              </label>
              <select v-model="form.preferred_call_time" class="can-exp-input"
                @change="clearErrors('preferred_call_time')">
                <option value="">{{ __("Select time...") }}</option>
                <option value="morning">{{ __("Morning (8am - 12pm)") }}</option>
                <option value="afternoon">{{ __("Afternoon (12pm - 5pm)") }}</option>
                <option value="evening">{{ __("Evening (5pm - 8pm)") }}</option>
              </select>
            ...
            <div class="relative w-full">
              <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg">
                {{ __("Preferred Call Date (Optional)") }}
              </label>
      ...
      <div class="bg-white rounded-lg overflow-hidden shadow-3xl my-6">
        ...
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="relative w-full">
              <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg">
                {{ __("Contact Person Name") }} <span class="text-red-500">*</span>
              </label>
              <input type="text" v-model="form.contact_name" class="can-exp-input bg-gray-50" placeholder="John Doe"
                readonly />
              <p class="text-xs text-gray-500 mt-1">{{ __("From your account") }}</p>
            </div>
            ...
            <div class="relative w-full md:col-span-2">
              ...
              <p class="text-xs text-gray-500 mt-1">{{ __("Enter your contact number for this sponsorship (max 15 characters, numbers and + only)") }}</p>
      ...
      <div class="bg-white rounded-lg overflow-hidden shadow-3xl my-6">
        <div class="px-4 py-3 sm:px-6 text-left bg-gradient-to-r from-primary via-primary to-secondary rounded-t-md">
          <h4 class="text-white">{{ __("Sponsorship Information") }}</h4>
        </div>
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="relative w-full md:col-span-2">
              <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg">
                {{ __("Company/Business Name") }} <span class="text-red-500">*</span>
              </label>
              <input type="text" v-model="form.company_name" class="can-exp-input" placeholder="Your Company Inc."
                @input="clearErrors('company_name')" />
              <p class="text-sm text-gray-500 mt-1">{{ __("This can be a different business than your previous sponsorships") }}</p>
              <Error v-if="submitted" fieldName="company_name" :validationErros="validationErros" />
            </div>

            <div class="relative w-full md:col-span-2">
              <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg">
                {{ __("Company Website (Optional)") }}
              </label>
            ...
            <div class="relative w-full md:col-span-2">
              ...
              <textarea v-model="form.summary" class="can-exp-input" rows="3"
                :placeholder="__('Brief description of your company and sponsorship...')"
            ...
            <div class="relative w-full md:col-span-2">
              ...
              <textarea v-model="form.detail_description" class="can-exp-input" rows="5"
                :placeholder="__('Detailed description of your company, products, or services...')"
      ...
      <div class="bg-white rounded-lg overflow-hidden shadow-3xl my-6">
        <div class="px-4 py-3 sm:px-6 text-left bg-gradient-to-r from-primary via-primary to-secondary rounded-t-md">
          <h4 class="text-white">{{ __("Images (Optional)") }}</h4>
        </div>
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg">{{ __("Company Logo") }}</label>
            ...
            <div>
              <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg">{{ __("Featured Image") }}</label>
              <file-pond name="featured_image" ref="featured_image" class="filepond"
                :label-idle="labelIdleFeatured"
                accepted-file-types="image/jpeg, image/png, image/jpg" :allow-multiple="false" :server="serverOptions"
                v-on:processfile="handleFeaturedProcessed" v-on:removefile="handleFeaturedRemoved" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-between items-center pt-6 border-t">
        <a :href="sponsorSettingsUrl" class="text-gray-600 hover:text-gray-800">
          ← {{ __("Back to My Sponsorships") }}
        </a>
        <button type="submit" class="button-exp-fill" :disabled="processing">
          <span v-if="processing">{{ __("Processing...") }}</span>
          <span v-else-if="form.talk_to_us_first">{{ __("Submit Contact Request") }}</span>
          <span v-else>{{ __("Complete Payment & Add Sponsorship") }}</span>
        </button>
      </div>
    </form>
  </div>
    ...
    async processPayment() {
      ...
      try {
        ...
        if (!this.form.talk_to_us_first && this.form.payment_method === "stripe" && this.form.sponsorship_amount > 0) {
          if (!this.cardElement) {
            helper.swalErrorMessageForWeb(this.__("Please enter card details"));
        ...
        if (response.data.status === "Success") {
          ...
          helper.swalSuccessMessageForWeb(
            response.data.message || this.__("Sponsorship added successfully!")
          );
