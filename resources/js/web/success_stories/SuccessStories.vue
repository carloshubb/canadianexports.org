    ...
    <form class="lg:w-full" id="contact-us-form" @submit.prevent="recaptcha()">
            ...
            <div class="grid mt-4 md:grid-cols-2 gap-4">
                <div class="relative w-full mb-3">
                    <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg" for="name">{{
                        JSON.parse(success_stories_setting)["name_label"] }}
                        <span class="text-red-500">*</span>
                    </label>
                    <input @input="clearErrors('name')" type="text" class="can-exp-input" :placeholder="JSON.parse(success_stories_setting)['name_placeholder']
                ...
                <div class="relative w-full mb-3">
                    <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg" for="email">{{
                        JSON.parse(success_stories_setting)["email_label"] }}
                        <span class="text-red-500">*</span>
                    </label>
                    <input type="text" class="can-exp-input" @input="fetchCustomerData" :placeholder="JSON.parse(success_stories_setting)['email_placeholder']
                ...
                <div class="relative w-full mb-3">
                    <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg" for="company_name">{{
                        JSON.parse(success_stories_setting)["company_name_label"] }}
                        <span class="text-red-500">*</span>
                    </label>
                    <input @input="clearErrors('company_name')" type="text" class="can-exp-input" :placeholder="JSON.parse(success_stories_setting)['company_name_placeholder']
                ...
                <div class="relative w-full mb-3">
                    <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg" for="business_categories">
                        {{ JSON.parse(success_stories_setting)["business_categories_label"] }}
                        <span class="text-red-500">*</span>
                    </label>
                    <input @input="updateBusinessCategories($event.target.value)" type="text" class="can-exp-input"
                        :placeholder="JSON.parse(success_stories_setting)['business_categories_placeholder']"
                ...
                <div class="relative w-full mb-3 col-span-2">
                    <label class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg" for="message">{{
                        JSON.parse(success_stories_setting)["message_label"] }}
                        <span class="text-red-500">*</span>
                    </label>
                    <textarea @input="clearErrors('message')" rows="5" type="text" class="can-exp-input" :placeholder="JSON.parse(success_stories_setting)['message_placeholder']
        ...
        <div class="mt-8 flex justify-center items-center">
            <button aria-label="Candian Exporters" class="button-exp-fill" type="submit" id="send-message">
                {{ JSON.parse(success_stories_setting)["button_text"] }}
            </button>
        </div>
...
export default {
    props: ["success_stories_setting", "submit_url", "page_id"],
    components: {
        Error,
        // ListErrors,
    },
    ...
    watch: {
        ...
        form: {
            handler(newValue) {
                localStorage.setItem("successStoriesFormData", JSON.stringify(newValue));
            },
    ...
    methods: {
        ...
        loadFormData() {
            const savedFormData = localStorage.getItem("successStoriesFormData");
            if (savedFormData) {
        ...
        async fetchCustomerData() {
            ...
            try {
                ...
                if (response.data.status === "success") {
                    ...
                    // Map the business categories to an array of objects
        ...
        clearForm() {
            ...
            localStorage.removeItem("successStoriesFormData");
        },
        async recaptcha() {
            this.submitted = true;
            this.loading = 1;
        ...
        saveForm() {
            ...
            // Ensure business_categories is an array of objects
