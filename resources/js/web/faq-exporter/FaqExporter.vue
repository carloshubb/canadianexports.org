<template>
    <form class="lg:w-full" id="contact-us-form" @submit.prevent="recaptcha()">
        ...
        <div class="grid mt-4 md:grid-cols-2 gap-4">
          <div class="relative w-full mb-3">
            <label
              ...
              for="name"
              >{{ JSON.parse(faq_exporter_setting)["name_label"] }}
            ...
            <input
             @input="clearErrors('name')"
              ...
              class="can-exp-input"
              :placeholder="
                JSON.parse(faq_exporter_setting)['name_placeholder']
              "
              id="name"
              v-model="form.name"
            />
          ...
          <div class="relative w-full mb-3">
            <label
              class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg"
              for="email"
              >{{ JSON.parse(faq_exporter_setting)["email_label"] }}
              <!-- <span class="text-red-500">*</span> -->
            </label>
            <input
             @input="clearErrors('email')"
              ...
              class="can-exp-input"
              :placeholder="
                JSON.parse(faq_exporter_setting)['email_placeholder']
              "
              id="email"
              v-model="form.email"
            />
          ...
          <div class="relative w-full mb-3 col-span-2">
            <label
              class="block text-gray-900 mb-2 text-base md:text-base lg:text-lg"
              for="message"
              >{{ JSON.parse(faq_exporter_setting)["message_label"] }}
              <span class="text-red-500">*</span>
            </label>
            <textarea
             @input="clearErrors('message')"
              ...
              class="can-exp-input"
              :placeholder="
                JSON.parse(faq_exporter_setting)['message_placeholder']
              "
              id="message"
      ...
      <div class="mt-8 flex justify-center items-center">
        <button
          ...
          id="send-message"
        >
          {{ JSON.parse(faq_exporter_setting)["button_text"] }}
        </button>
      </div>
  ...
  export default {
    props: ["faq_exporter_setting", "submit_url", "page_id"],
    components: {
      Error,
      // ListErrors,
    },
  ...
  watch: {
    ...
    form: {
      handler(newValue) {
        localStorage.setItem("faqExporterFormData", JSON.stringify(newValue));
      },
    ...
    methods: {
        loadFormData() {
      const savedFormData = localStorage.getItem("faqExporterFormData");
      if (savedFormData) {
      ...
      clearForm() {
        ...
        localStorage.removeItem("faqExporterFormData");
      },
      async recaptcha() {
          this.submitted = true;
        this.loading = 1;
