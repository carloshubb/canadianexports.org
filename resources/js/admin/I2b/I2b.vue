      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
          <div class="sm:flex-auto">
            <h1 class="can-exp-h3 mb-0 text-primary">Inquiries to buy</h1>
            <p class="mt-2 text-sm text-gray-700">
              A list of all the Inquiries to buy
            </p>
          </div>
          <div class="mt-4 flex md:ml-4 md:mt-0">
            <router-link
              :to="{ name: 'admin.i2b.create' }"
              class="button-exp-fill"
            >
              Add new inquiry
            </router-link>
            <router-link
              :to="{ name: 'admin.i2b.import' }"
            ...
            >
              <table class="min-w-full divide-y divide-gray-300">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                  <tr class="hover:shadow-lg" v-for="i2 in i2b" :key="i2.id">
                    <td
                      class="w-full max-w-0 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:w-auto sm:max-w-none sm:pl-1"
                    >
                      {{
                        i2.i2b_detail && i2.i2b_detail[0]
                          ? i2.i2b_detail[0].name
                          : ""
                      }}
                      <dl class="font-normal lg:hidden">
                        <dt class="sr-only">Business category</dt>
                        <dd class="mt-1 truncate text-gray-700">
                          {{ i2.business_category_name }}
                        </dd>
                        <dt class="sr-only">Country</dt>
                        <dd class="mt-1 truncate text-gray-700">
                          {{
                            i2.i2b_detail && i2.i2b_detail[0]
                              ? i2.i2b_detail[0].country_name
                              : ""
                          }}
                        </dd>
                    ...
                    <td
                      class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell"
                    >
                      {{ formatCurrency(i2.estimated_value) }}
                    </td>
                    <td
                      class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell"
                    >
                ...
                <div>
                  ...
                  >
                    <a
                      ...
                      @click="fetchI2b(pagination.prev_page_url)"
                    >
                    ...
                    <a
                      ...
                      @click="fetchI2b(pagination.next_page_url)"
                    >
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.i2b.limit,
      form: (state) => state.i2b.form,
      i2b: (state) => state.i2b.i2b,
      pagination: (state) => state.i2b.pagination,
      validationErros: (state) => state.i2b.validationErros,
      searchParam: (state) => state.i2b.searchParam,
  ...
  methods: {
    ...
    fetchI2b(page_url) {
      this.$store.dispatch("i2b/fetchI2b", { url: page_url });
    },
    updateLimit(value) {
      this.$store.commit("i2b/setLimit", value);
      this.$store.dispatch("i2b/fetchI2b");
    },
    deleteI2b(item) {
  this.$swal
    .fire({
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        this.$swal
          .fire({
            title: "Are you sure you want to delete this I2B?",
            icon: "warning",
            showCancelButton: true,
                ...
                .then((res) => {
                  if (res.data.status === "Success") {
                    helper.swalSuccessMessage(
                      res.data.message || "I2B deleted successfully"
                    );
                    this.$store.dispatch("i2b/fetchI2b");
                  } else {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("i2b/setSearchParam", this.quickSearch);
      this.$store.dispatch("i2b/fetchI2b");
    }, 500),
    actionModal(id) {
      this.showModalId = id;
  ...
  created() {
    this.$store.commit("i2b/setLimit", 100);
    this.$store.commit("i2b/setSortBy", "i2b_name");
    this.$store.commit("i2b/setSortType", "asc");
