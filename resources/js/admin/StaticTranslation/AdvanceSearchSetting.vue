<template>
    <div class="my-5" v-for="language in languages" :key="language.id" :class="{ 'block': shouldShowLanguage(language), 'hidden': !shouldShowLanguage(language) }">
      <div v-if="hasStaticTranslationDetail(language)" class="grid my-5 grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="field in ['advance_search_heading', 'search_input_label', 'search_input_placeholder', 'search_input_error', 'search_cat_select_category_label', 'canadian_exporters_text', 'i2b_text', 'events_text', 'all_option_text', 'select_industry_label', 'select_industry_error', 'search_type_btn_text', 'search_results_for_text', 'featured_search_results_for_text', 'premium_search_results_for_text', 'i2b_search_results_for_text', 'events_search_results_for_text', 'ce_sorting_order_label', 'i2b_sorting_order_label', 'events_sorting_order_label', 'alphabetical_order_a_z', 'alphabetical_order_z_a', 'include_expired', 'display_past_events', 'date_created_asc', 'date_created_desc']" :key="field" class="relative z-0 w-full group">
