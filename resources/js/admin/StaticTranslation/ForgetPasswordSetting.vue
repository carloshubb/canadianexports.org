<template>
  ...
  >
    ...
    >
      <!-- <div v-for="field in ['header_heading', 'header_text', 'email_label_text', 'email_error_text', 'submit_button_text']" :key="field" class="relative z-0 w-full group">
      ...
      >
        ...
        <template v-if="field === 'header_text'">
          <textarea
            :name="`${generateFieldName(field, language)}`"
            :id="`${generateFieldId(field, language)}`"
            class="can-exp-input w-full block border border-gray-300 rounded"
            ...
            "
            v-text="getFieldValue(field, language)"
          ></textarea>
        </template>
        <template v-else>
          <input
