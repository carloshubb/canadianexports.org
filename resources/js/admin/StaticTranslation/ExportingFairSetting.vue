<template>
  <div class="my-5" v-for="language in languages" :key="language.id"
    :class="{ 'block': shouldShowLanguage(language), 'hidden': !shouldShowLanguage(language) }">
    <div v-if="hasStaticTranslationDetail(language)"
      class="grid my-5 grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
        v-for="field in ['ef_event_date_text', 'ef_venue_text', 'ef_location_text', 'ef_short_description_text', 'ef_booth_number_text', 'ef_product_search_text', 'ef_for_exibitor_url_text', 'ef_for_visitor_url_text', 'ef_for_press_url_text', 'ef_contacts_text', 'ef_visit_event_website_text', 'ef_contact_information_modal_title', 'ef_contact_information_modal_name', 'ef_contact_information_modal_email', 'ef_contact_information_modal_phone', 'ef_contact_information_modal_designation', 'ef_event_detail_button_text']"
        :key="field" class="relative z-0 w-full group">
        <label :for="`${generateFieldId(field, language)}`">{{ getDisplayText(field, language) }}</label>
        <input type="text" :name="`${generateFieldName(field, language)}`" :id="`${generateFieldId(field, language)}`"
        ...
        <p class="mt-2 text-sm text-red-400" v-if="hasValidationError(field, language)"
          v-text="getValidationError(field, language)"></p>
      </div>
    </div>
  </div>
</template>
