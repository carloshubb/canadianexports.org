<template>
    <div class="my-5" v-for="language in languages" :key="language.id" :class="{ 'block': shouldShowLanguage(language), 'hidden': !shouldShowLanguage(language) }">
      <div v-if="hasStaticTranslationDetail(language)" class="grid my-5 grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="field in ['event_date_text', 'venue_text', 'location_text', 'product_search_text', 'for_exibitor_url_text', 'for_visitor_url_text', 'for_press_url_text', 'video_url_text', 'description_text', 'book_a_stand_text', 'organizer_contact_heading', 'contact_email_text', 'contact_phone_text']" :key="field" class="relative z-0 w-full group">
