      ...
      <header class="pt-4">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center justify-between">
            ...
            <router-link :to="{ name: 'admin.successStories.index' }" class="button-exp-fill">
              Back
            </router-link>
          </div>
        </div>
        ...
        <div class="grid my-5 grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6"
          ...
          <div class="relative z-0 w-full group">
            ...
            <input type="text" name="company_name" id="company_name"
              ...
              @input="handleCompanyNameInput($event.target.value, language)" :value="form['company_name'] && form['company_name'][`company_name_${language.id}`]
                  ? form['company_name'][`company_name_${language.id}`]
                  : ''
                " />
            <p class="mt-2 text-sm text-red-400" v-if="validationErros.has(`company_name.company_name_${language.id}`)"
              v-text="validationErros.get(`company_name.company_name_${language.id}`)"></p>
          </div>
          <div class="relative z-0 w-full group col-span-2">
            <label for="message">Message</label>
            <textarea name="message" id="message" class="can-exp-input w-full block border border-gray-300 rounded"
              placeholder=" " @input="handleMessageInput($event.target.value, language)" v-text="form['message'] && form['message'][`message_${language.id}`]
                  ? form['message'][`message_${language.id}`]
                  : ''
                "></textarea>
            <p class="mt-2 text-sm text-red-400" v-if="validationErros.has(`message.message_${language.id}`)"
              v-text="validationErros.get(`message.message_${language.id}`)"></p>
          </div>
        </div>
        <div class="grid my-5 grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
          <div class="relative z-0 w-full group">
