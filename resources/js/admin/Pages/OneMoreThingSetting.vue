        ...
        <div class="border rounded w-full" :class="collapseStates[0] ? 'bg-blue-50' : ''">
                    ...
                    <li class="relative overflow-hidden lg:flex-1">
                        ...
                        <div class="group w-full">
                            ...
                            <span class="ml-4 mt-0.5 flex min-w-0 flex-col">
                                ...
                                <span class="text-sm font-medium text-gray-500">One more thing page settings</span>
                            </span>
                            </span>
                        </div>
            ...
            <div class="p-4 bg-gray-50 border-t" v-show="collapseStates[0]">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    ...
                    <div class="relative z-0 w-full group">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`read_more_btn_text_${selectedLanguage}`">Read more button text</label>
                        <input type="text" :name="`read_more_btn_text_${selectedLanguage}`"
                            :id="`read_more_btn_text_${selectedLanguage}`"
                            class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6"
                            placeholder=" "
                            @input="
                            handleInput(
                        ...
                        "
                            :value="form['read_more_btn_text'] &&
                                form['read_more_btn_text'][
                                    `read_more_btn_text_${selectedLanguage}`
                                ] ?
                                form['read_more_btn_text'][
                                    `read_more_btn_text_${selectedLanguage}`
                                ] :
                        ...
                        <p class="mt-2 text-sm text-red-400"
                            v-if="
                                validationErros.has(
                                    `read_more_btn_text.read_more_btn_text_${selectedLanguage}`
                                )
                            "
                            v-text="
                                validationErros.get(
    ...
    methods: {
        ...
        fetchPageSetting() {
            this.$store
                .dispatch("pages/fetchPage", {
                    url: `${process.env.MIX_ADMIN_API_URL}one-more-thing-setting?withOneMoreThingSettingDetail=1&findByPageId=${this.form.id}`,
                })
                .then((res) => {
                    let data = res.data.data && res.data.data.one_more_thing_setting_detail
                            ? res.data.data.one_more_thing_setting_detail
                            : [];
                    ...
                    obj = {};
                    data.map((res) => {
                        obj["read_more_btn_text_" + res.language_id] = res.read_more_btn_text;
                    });
                    this.$store.commit("pages/setHomePageSetting", {
                        key: "read_more_btn_text",
                        value: obj,
                    });
                });
        },
        checkValidationError(validationErros, language) {
            return (
                ...
                validationErros.has(`read_more_btn_text.read_more_btn_text_${language.id}`)
            );
        },
    },
    created() {
        this.$store
            ...
            .then((res) => {
                ...
                obj = {};
                data.map((res) => {
                    obj["read_more_btn_text_" + res.id] = "";
                });
                this.$store.commit("pages/setHomePageSetting", {
                    key: "read_more_btn_text",
                    value: obj,
