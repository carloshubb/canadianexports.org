                    ...
                    <li class="relative overflow-hidden lg:flex-1">
                        ...
                        <div class="group w-full">
                            ...
                            <span class="ml-4 mt-0.5 flex min-w-0 flex-col">
                                <span class="text-sm font-medium">User events settings</span>
                                <span class="text-sm font-medium text-gray-500">User events page settings</span>
                            </span>
                            </span>
                        </div>
            ...
            <div class="p-4 bg-gray-50 border-t" v-show="collapseStates[0]">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    <div class="relative z-0 w-full group">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`no_event_found_text_${selectedLanguage}`">No event found text</label>
                        <input type="text" :name="`no_event_found_text_${selectedLanguage}`"
                            :id="`no_event_found_text_${selectedLanguage}`"
                        ...
                        "
                            :value="form['no_event_found_text'] &&
                                form['no_event_found_text'][
                                    `no_event_found_text_${selectedLanguage}`
                                ] ?
                                form['no_event_found_text'][
                                    `no_event_found_text_${selectedLanguage}`
                                ] :
                        ...
                        <p class="mt-2 text-sm text-red-400"
                            v-if="
                                validationErros.has(
                                    `no_event_found_text.no_event_found_text_${selectedLanguage}`
                                )
                            "
                            v-text="
                                validationErros.get(
                    ...
                    <div class="relative z-0 w-full group">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`user_has_events_text_${selectedLanguage}`">User has events text</label>
                        <input type="text" :name="`user_has_events_text_${selectedLanguage}`"
                            :id="`user_has_events_text_${selectedLanguage}`"
                            class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6"
                            placeholder=" "
                            @input="
                            handleInput(
                                $event.target.value,
                                language,
                                'user_has_events_text',
                                'updateHomePageSetting'
                            )
                        "
                            :value="form['user_has_events_text'] &&
                                form['user_has_events_text'][
                                    `user_has_events_text_${selectedLanguage}`
                                ] ?
                                form['user_has_events_text'][
                                    `user_has_events_text_${selectedLanguage}`
                                ] :
                        ...
                        <p class="mt-2 text-sm text-red-400"
                            v-if="
                                validationErros.has(
                                    `user_has_events_text.user_has_events_text_${selectedLanguage}`
                                )
                            "
                            v-text="
                                validationErros.get(
                    ...
                    <div class="relative z-0 w-full group">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`add_event_btn_text_${selectedLanguage}`">Add event button text</label>
                        <input type="text" :name="`add_event_btn_text_${selectedLanguage}`"
                            :id="`add_event_btn_text_${selectedLanguage}`"
                            class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6"
                            placeholder=" "
                            @input="
                            handleInput(
                        ...
                        "
                            :value="form['add_event_btn_text'] &&
                                form['add_event_btn_text'][
                                    `add_event_btn_text_${selectedLanguage}`
                                ] ?
                                form['add_event_btn_text'][
                                    `add_event_btn_text_${selectedLanguage}`
                                ] :
                        ...
                        <p class="mt-2 text-sm text-red-400"
                            v-if="
                                validationErros.has(
                                    `add_event_btn_text.add_event_btn_text_${selectedLanguage}`
                                )
                            "
                            v-text="
                                validationErros.get(
                    ...
                    <div class="relative z-0 w-full group">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`events_text_${selectedLanguage}`">Events text</label>
                        <input type="text" :name="`events_text_${selectedLanguage}`"
                            :id="`events_text_${selectedLanguage}`"
                            class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6"
                            placeholder=" "
                            @input="
                            handleInput(
                        ...
                        <p class="mt-2 text-sm text-red-400"
                            v-if="
                                validationErros.has(
                                    `events_text.events_text_${selectedLanguage}`
                                )
                            "
                            v-text="
                                validationErros.get(
...
export default {
    ...
    methods: {
        ...
        fetchPageSetting() {
            this.$store
                .dispatch("pages/fetchPage", {
                    url: `${process.env.MIX_ADMIN_API_URL}event-listing-setting?withEventListingSettingDetail=1&findByPageId=${this.form.id}`,
                })
                .then((res) => {
                    let data = res.data.data && res.data.data.event_listing_setting_detail
                            ? res.data.data.event_listing_setting_detail
                            : [];

                    let obj = {};
                    data.map((res) => {
                        obj["no_event_found_text_" + res.language_id] = res.no_event_found_text;
                    });
                    this.$store.commit("pages/setHomePageSetting", {
                        key: "no_event_found_text",
                        value: obj,
                    });
                    obj = {};
                    data.map((res) => {
                        obj["user_has_events_text_" + res.language_id] = res.user_has_events_text;
                    });
                    this.$store.commit("pages/setHomePageSetting", {
                        key: "user_has_events_text",
                        value: obj,
                    });
                    data.map((res) => {
                        obj["add_event_btn_text_" + res.language_id] = res.add_event_btn_text;
                    });
                    this.$store.commit("pages/setHomePageSetting", {
                        key: "add_event_btn_text",
                        value: obj,
                    });
                    data.map((res) => {
                    ...
                    data.map((res) => {
                        obj["events_text_" + res.language_id] = res.events_text;
                    });
                    this.$store.commit("pages/setHomePageSetting", {
                        key: "events_text",
                        value: obj,
        ...
        checkValidationError(validationErros, language) {
            return (
                validationErros.has(`no_event_found_text.no_event_found_text_${language.id}`) ||
                validationErros.has(`user_has_events_text.user_has_events_text_${language.id}`) ||
                validationErros.has(`add_event_btn_text.add_event_btn_text_${language.id}`) ||
                ...
                validationErros.has(`events_text.events_text_${language.id}`) ||
    ...
    created() {
        ...
        this.$store
            ...
            .then((res) => {
                ...
                let obj = {};
                data.map((res) => {
                    obj["no_event_found_text_" + res.id] = "";
                });
                this.$store.commit("pages/setHomePageSetting", {
                    key: "no_event_found_text",
                    value: obj,
                ...
                obj = {};
                data.map((res) => {
                    obj["user_has_events_text_" + res.id] = "";
                });
                this.$store.commit("pages/setHomePageSetting", {
                    key: "user_has_events_text",
                    value: obj,
                ...
                data.map((res) => {
                    obj["add_event_btn_text_" + res.id] = "";
                });
                this.$store.commit("pages/setHomePageSetting", {
                    key: "add_event_btn_text",
                    value: obj,
