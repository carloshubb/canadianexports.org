        ...
        <div class="border rounded w-full" :class="collapseStates[0] ? 'bg-gray-50' : ''">
            ...
            <div class="p-4 bg-gray-100 border-t" v-show="collapseStates[0]">
                <div class="grid my-5 md:gap-6 gap-4">
                    ...
                    <div class="relative z-0 w-full group">
                        <label :for="`contact_label_${selectedLanguage}`">Contact label</label>
                        <input type="text" :name="`contact_label_${selectedLanguage}`"
                            :id="`contact_label_${selectedLanguage}`"
                            class="can-exp-input w-full block border border-gray-300 rounded"
                            placeholder=" "
                            @input="
                            handleInput(
                        ...
                        "
                            :value="form['contact_label'] &&
                                form['contact_label'][
                                    `contact_label_${selectedLanguage}`
                                ] ?
                                form['contact_label'][
                                    `contact_label_${selectedLanguage}`
                                ] :
                        ...
                        <p class="mt-2 text-sm text-red-400"
                            v-if="
                                validationErros.has(
                                    `contact_label.contact_label_${selectedLanguage}`
                                )
                            "
                            v-text="
                                validationErros.get(
                    ...
                    <div class="relative z-0 w-full group">
                        <label :for="`button_text_${selectedLanguage}`">Submit button text</label>
    ...
    methods: {
        ...
        fetchPageSetting() {
            this.$store
                .dispatch("pages/fetchPage", {
                    url: `${process.env.MIX_ADMIN_API_URL}sponsor-page-setting?withSponsorPageSettingDetail=1&findByPageId=${this.form.id}`,
                })
                .then((res) => {
                    let data = res.data.data && res.data.data.sponsor_page_setting_detail
                            ? res.data.data.sponsor_page_setting_detail
                            : [];
                    ...
                    obj = {};
                    data.map((res) => {
                        obj["contact_label_" + res.language_id] = res.contact_label;
                    });
                    this.$store.commit("pages/setHomePageSetting", {
                        key: "contact_label",
                        value: obj,
        ...
        checkValidationError(validationErros, language) {
            return (
                ...
                validationErros.has(`contact_label.contact_label_${language.id}`) ||
        ...
        <div class="border rounded w-full" :class="collapseStates[0] ? 'bg-blue-50' : ''">
                    ...
                    <li class="relative overflow-hidden lg:flex-1">
                        ...
                        <div class="group w-full">
                            ...
                            <span class="ml-4 mt-0.5 flex min-w-0 flex-col">
                                ...
                                <span class="text-sm font-medium text-gray-500">Sponsor page settings</span>
                            </span>
                            </span>
                        </div>
                        <svg class="w-5 h-5 fill-current text-gray-500" viewBox="0 0 20 20">
            ...
            <div class="p-4 bg-gray-50 border-t" v-show="collapseStates[0]">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    ...
                    <div class="relative z-0 w-full group">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`email_label_${selectedLanguage}`">Title heading</label>
                        <input type="text" :name="`email_label_${selectedLanguage}`"
                            :id="`email_label_${selectedLanguage}`"
                    ...
                    <div class="relative z-0 w-full group">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`contact_label_${selectedLanguage}`">Contact label</label>
                        <input type="text" :name="`contact_label_${selectedLanguage}`"
                            :id="`contact_label_${selectedLanguage}`"
                            class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6"
                            placeholder=" "
                    ...
                    <div class="relative z-0 w-full group">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`button_text_${selectedLanguage}`">Deadline heading</label>
                        <input type="text" :name="`button_text_${selectedLanguage}`"
                            :id="`button_text_${selectedLanguage}`"
                            class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6"
                            placeholder=" "
