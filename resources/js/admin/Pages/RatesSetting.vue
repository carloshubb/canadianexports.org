                    ...
                    <li class="relative overflow-hidden lg:flex-1">
                        ...
                        <div class="group w-full">
                            ...
                            <span class="ml-4 mt-0.5 flex min-w-0 flex-col">
                                ...
                                <span class="text-sm font-medium text-gray-500">Rates page settings</span>
                            </span>
                            </span>
                        </div>
            ...
            <div class="p-4 bg-gray-50 border-t" v-show="collapseStates[0]">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    <div class="sm:col-span-6" v-if="isDataLoaded">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`price_table_pre_text_${selectedLanguage}`">Price table pre text</label>
                        <editor
                            @mouseleave="
                                handleSelectionChange(
                                    language,
                                    'price_table_pre_text',
                            ...
                            "
                            :ref="`price_table_pre_text_${language.id}`"
                            :id="`price_table_pre_text_${language.id}`"
                            :initial-value="form?.[`price_table_pre_text`]?.[`price_table_pre_text_${language?.id}`] || null
                            "
                        ...
                        <p class="mt-2 text-sm text-red-400"
                            v-if="
                                validationErros.has(
                                    `price_table_pre_text.price_table_pre_text_${selectedLanguage}`
                                )
                            "
                            v-text="
                                validationErros.get(
                    ...
                    <div class="sm:col-span-6" v-if="isDataLoaded">
                        <label class="block text-sm font-medium leading-6 text-gray-900" :for="`price_table_post_text_${selectedLanguage}`">Price table post text</label>
                        <editor
                            @mouseleave="
                                handleSelectionChange(
                                    language,
                                    'price_table_post_text',
                                    'updateHomePageSetting'
                                )
                            "
                            @keyup="
                            ...
                            "
                            :ref="`price_table_post_text_${language.id}`"
                            :id="`price_table_post_text_${language.id}`"
                            :initial-value="form?.[`price_table_post_text`]?.[`price_table_post_text_${language?.id}`] || null
                            "
                            :tinymce-script-src="tinymceScriptSrc"
                            :init="editorConfig"
                        />
                        ...
                        <p class="mt-2 text-sm text-red-400"
                            v-if="
                                validationErros.has(
                                    `price_table_post_text.price_table_post_text_${selectedLanguage}`
                                )
                            "
                            v-text="
                                validationErros.get(
...
export default {
    ...
    methods: {
        ...
        fetchPageSetting() {
            this.$store
                .dispatch("pages/fetchPage", {
                    url: `${process.env.MIX_ADMIN_API_URL}rates-setting?withRatesSettingDetail=1&findByPageId=${this.form.id}`,
                })
                .then((res) => {
                    let data = res.data.data && res.data.data.rates_setting_detail
                            ? res.data.data.rates_setting_detail
                            : [];

                    let obj = {};
                    data.map((res) => {
                        obj["price_table_pre_text_" + res.language_id] = res.price_table_pre_text;
                    });
                    this.$store.commit("pages/setHomePageSetting", {
                        key: "price_table_pre_text",
                        value: obj,
                    });
                    obj = {};
                    data.map((res) => {
                        obj["price_table_post_text_" + res.language_id] = res.price_table_post_text;
                    });
                    this.$store.commit("pages/setHomePageSetting", {
                        key: "price_table_post_text",
                        value: obj,
                    });
                    obj = {};
            data.map((res) => {
        ...
        checkValidationError(validationErros, language) {
            return (
                validationErros.has(`price_table_pre_text.price_table_pre_text_${language.id}`) ||
                validationErros.has(`price_table_post_text.price_table_post_text_${language.id}`) ||
    ...
    created() {
        ...
        this.$store
            ...
            .then((res) => {
                ...
                let obj = {};
                data.map((res) => {
                    obj["price_table_pre_text_" + res.id] = "";
                });
                this.$store.commit("pages/setHomePageSetting", {
                    key: "price_table_pre_text",
                    value: obj,
                ...
                obj = {};
                data.map((res) => {
                    obj["price_table_post_text_" + res.id] = "";
                });
                this.$store.commit("pages/setHomePageSetting", {
                    key: "price_table_post_text",
                    value: obj,
