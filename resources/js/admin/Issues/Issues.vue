      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
          <div class="sm:flex-auto">
            <h1 class="can-exp-h3 mb-0 text-primary">Magazine Issues</h1>
            <p class="mt-2 text-sm text-gray-700">
              A list of all the magazine issues
            </p>
          </div>
          <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
            <router-link
              :to="{ name: 'admin.issues.create' }"
                    ...
                    >
                      ...
                      <dl class="font-normal lg:hidden">
                        ...
                        <dd class="mt-1 truncate text-gray-500">
                          {{
                            issue.issue_detail && issue.issue_detail[0]
                              ? issue.issue_detail[0].title
                              : ""
                          }}
                        </dd>
                      </dl>
                    ...
                    >
                      <div class="flex items-center space-x-4">
                        ...
                        <a
                          ...
                          data-tooltip="Delete magazine"
                          @click.prevent="deleteIssue(issue)"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                ...
                <div>
                  ...
                  >
                    <a
                      ...
                      @click="fetchIssues(pagination.prev_page_url)"
                    >
                    ...
                    <a
                      ...
                      @click="fetchIssues(pagination.next_page_url)"
                    >
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.issues.limit,
      form: (state) => state.issues.form,
      issues: (state) => state.issues.issues,
      pagination: (state) => state.issues.pagination,
      validationErros: (state) => state.issues.validationErros,
      searchParam: (state) => state.issues.searchParam,
  ...
  methods: {
    fetchIssues(page_url) {
      this.$store.dispatch("issues/fetchIssues", { url: page_url });
    },
    updateLimit(value) {
      this.$store.commit("issues/setLimit", value);
      this.$store.dispatch("issues/fetchIssues");
    },
    deleteIssue(item) {
  this.$swal
    .fire({
 ...
 quickSearchFilter: _.debounce(function () {
      this.$store.commit("issues/setSearchParam", this.quickSearch);
      this.$store.dispatch("issues/fetchIssues");
    }, 500),
    actionModal(id) {
      this.showModalId = id;
  ...
  created() {
    this.$store.commit("issues/setLimit", 100);
    this.$store.commit("issues/setSortBy", "issue_title");
    this.$store.commit("issues/setSortType", "asc");
