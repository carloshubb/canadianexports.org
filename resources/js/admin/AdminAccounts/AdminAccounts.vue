      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
          <div class="sm:flex-auto">
            <h1 class="can-exp-h3 mb-0 text-primary">Admin accounts</h1>
            <p class="mt-2 text-sm text-gray-700">
              A list of all the users in system which are created by Canadian
              Export admin.
            </p>
          </div>
          <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
            <router-link
              :to="{ name: 'admin.admin-accounts.create' }"
        ...
        <div class="mt-8 flow-root">
          ...
          >
            ...
            <div class="relative w-full md:w-auto">
              ...
              <input
                type="text"
                id="table-search-admin_accounts"
              ...
              <table class="min-w-full divide-y divide-gray-300">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                  <tr
                    ...
                    v-for="admin_account in admin_accounts"
                    :key="admin_account.id"
                  >
                    <td
                    ...
                    >
                      ...
                      <dl class="font-normal lg:hidden">
                        ...
                        <dd class="mt-1 truncate text-gray-500">
                          {{ formatDate(admin_account.updated_at) }}
                        </dd>
                    ...
                    >
                      <div class="text-gray-900">{{ admin_account.email }}</div>
                    </td>
                    <td
                      class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell"
                    >
                      {{ formatDate(admin_account.created_at) }}
                    </td>
                    <td
                      class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell"
                    >
                    ...
                    >
                      <div class="flex items-center space-x-4">
                        ...
                        <a
                          ...
                          @click.prevent="deleteAdminAccount(admin_account)"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                ...
                <div>
                  ...
                  >
                    <a
                      ...
                      @click="fetchAdminAccounts(pagination.prev_page_url)"
                    >
                    ...
                    <a
                      ...
                      @click.prevent="
                        fetchAdminAccounts(pagination.next_page_url)
                      "
                    >
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.admin_accounts.limit,
      form: (state) => state.admin_accounts.form,
      admin_accounts: (state) => state.admin_accounts.admin_accounts,
      pagination: (state) => state.admin_accounts.pagination,
      validationErros: (state) => state.admin_accounts.validationErros,
      searchParam: (state) => state.admin_accounts.searchParam,
      loading: (state) => state.admin_accounts.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    fetchAdminAccounts(page_url) {
      this.$store.dispatch("admin_accounts/fetchAdminAccounts", {
        url: page_url,
      });
    },
    updateLimit(value) {
      this.$store.commit("admin_accounts/setLimit", value);
      this.$store.dispatch("admin_accounts/fetchAdminAccounts");
    },
    deleteAdminAccount(item) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("admin_accounts/deleteAdminAccount", {
                id: item.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("admin_accounts/setSearchParam", this.quickSearch);
      this.$store.dispatch("admin_accounts/fetchAdminAccounts");
    }, 500),
    actionModal(id) {
      this.showModalId = id;
  ...
  created() {
    this.$store.commit("admin_accounts/setLimit", 100);
    this.$store.commit("admin_accounts/setSortBy", "name");
    this.$store.commit("admin_accounts/setSortType", "asc");
