            ...
            <header class="pt-4">
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <h2 class="can-exp-h3 text-primary"> {{ isFormEdit ? 'Edit' : 'Create' }} admin account </h2>
                ...
                <div class="grid my-5 grid-cols-1 sm:grid-cols-1 md:grid-cols-2  lg:grid-cols-2 gap-6 ">
                    ...
                    <div class="relative z-0 w-full group">
                        ...
                        <input type="email" name="email" id="email"
                            class="can-exp-input w-full block border border-gray-300 rounded" placeholder=" "
                            :value="form.email" @input="updateForm('email', $event.target.value)" />
                    ...
                    <div class="relative z-0 w-full group">
                        ...
                        <div class="relative">
                            <input :type="display_password" name="password" id="password"
                                class="can-exp-input w-full block border border-gray-300 rounded" placeholder=" "
                                :value="form.password" @input="updateForm('password', $event.target.value)" />
                            ...
                            <svg class="w-5 h-5 text-gray-500 absolute top-3 right-3"
                    ...
                    <div class="relative z-0 w-full group">
                        ...
                        <div class="relative">
                            <input :type="display_confirm_password" name="password_confirmation"
                                id="password_confirmation"
                                class="can-exp-input w-full block border border-gray-300 rounded" placeholder=" "
                                :value="form.password_confirmation"
                                @input="updateForm('password_confirmation', $event.target.value)" />
                            <svg class="w-5 h-5 text-gray-500 absolute top-3 right-3"
...
export default {
    computed: {
        ...mapState({
            ...
            isFormEdit: state => state.admin_accounts.isFormEdit,
    ...
    methods: {
        updateForm(field, value) {
            ...
            this.$store.commit('admin_accounts/updateForm', {
                [field]: value
            });
        },
        addUpdateForm() {
            this.$store.dispatch('admin_accounts/addUpdateForm')
                .then(() => this.$router.push({ name: 'admin.admin-accounts.index' }));
        },
    },
    created() {
        this.$store.commit('admin_accounts/resetForm');
        if (this.$route.params.id) {
            let id = this.$route.params.id;
            this.$store.commit('admin_accounts/setIsFormEdit', 1);
            this.$store.dispatch('admin_accounts/fetchAdminAccount', { id: id }).then(res => {
