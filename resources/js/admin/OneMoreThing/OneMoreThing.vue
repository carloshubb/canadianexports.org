      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
          <div class="sm:flex-auto">
            <h1 class="can-exp-h3 mb-0 text-primary">One more thing</h1>
            <p class="mt-2 text-sm text-gray-700">
              A list of all the One more thing page
            </p>
          </div>
          <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
            <router-link
              :to="{ name: 'admin.one-more-thing.create' }"
        ...
        <div class="mt-8 flow-root">
          ...
          >
            ...
            <div class="relative w-full md:w-auto">
              ...
              <input
                type="text"
                id="table-search-one-more-thing"
              ...
              <table class="min-w-full divide-y divide-gray-300">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                  <tr
                    ...
                    v-for="one_thing in one_more_thing"
                    :key="one_thing.id"
                  >
                    <td
                    ...
                    >
                      <img
                        v-if="one_thing.media"
                        class="w-10 h-10 rounded-full"
                        :src="`/${one_thing.media.path}`"
                        alt="one more thing"
                      />
                      <dl class="font-normal lg:hidden">
                        ...
                        <dd class="mt-1 truncate text-gray-500">
                          {{
                            one_thing.one_more_thing_detail &&
                            one_thing.one_more_thing_detail[0]
                              ? one_thing.one_more_thing_detail[0].description
                              : ""
                          }}
                        </dd>
                      </dl>
                    ...
                    >
                      <div>
                        <span v-if="!one_thing.showFullMessage">
                          {{
                            truncateMessage(
                              one_thing.one_more_thing_detail &&
                                one_thing.one_more_thing_detail[0]
                        ...
                        <a
                          ...
                          "
                          href="#"
                          @click.prevent="toggleMessage(one_thing)"
                          class="text-blue-600 hover:text-blue-800 text-sm"
                        >
                          {{
                            one_thing.showFullMessage
                              ? "Show Less"
                              : "Read More"
                          }}
                        </a>
                      ...
                      <div class="flex items-center space-x-4">
                        <router-link
                          :to="{
                            ...
                            params: { id: one_thing.id },
                          }"
                          data-tooltip="Edit one more thing"
                        ...
                        <a
                          ...
                          data-tooltip="Delete one more thing"
                          @click.prevent="deleteOneMoreThing(one_thing)"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                ...
                <div>
                  ...
                  >
                    <a
                      ...
                      @click="fetchOneMoreThings(pagination.prev_page_url)"
                    >
                    ...
                    <a
                      ...
                      @click="fetchOneMoreThings(pagination.next_page_url)"
                    >
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.one_more_thing.limit,
      form: (state) => state.one_more_thing.form,
      one_more_thing: (state) => state.one_more_thing.one_more_thing,
      pagination: (state) => state.one_more_thing.pagination,
      validationErros: (state) => state.one_more_thing.validationErros,
      searchParam: (state) => state.one_more_thing.searchParam,
  ...
  methods: {
    truncateMessage(message) {
      return message.split(" ").slice(0, 15).join(" ");
    },
    toggleMessage(form) {
      form.showFullMessage = !form.showFullMessage;
    },
    isLongMessage(message) {
      return message.split(" ").length > 15;
    },
    fetchOneMoreThings(page_url) {
      this.$store.dispatch("one_more_thing/fetchOneMoreThings", {
        url: page_url,
      });
    },
    updateLimit(value) {
      this.$store.commit("one_more_thing/setLimit", value);
      this.$store.dispatch("one_more_thing/fetchOneMoreThings");
    },
    deleteOneMoreThing(item) {
  this.$swal
    .fire({
        ...
        this.$swal
          ...
          .then((confirmResult) => {
            if (confirmResult.isConfirmed) {
              ...
              this.$store
                .dispatch("one_more_thing/deleteOneMoreThing", {
                  id: item.id,
...
quickSearchFilter: _.debounce(function () {
      this.$store.commit("one_more_thing/setSearchParam", this.quickSearch);
      this.$store.dispatch("one_more_thing/fetchOneMoreThings");
    }, 500),
    actionModal(id) {
      this.showModalId = id;
  ...
  created() {
    this.$store.commit("one_more_thing/setLimit", 100);
    this.$store.commit("one_more_thing/setSortBy", "detail_description");
    this.$store.commit("one_more_thing/setSortType", "asc");
    this.$store.commit("one_more_thing/setSearchParam", "");
    this.$store.dispatch("one_more_thing/fetchOneMoreThings");
  },
  watch: {
