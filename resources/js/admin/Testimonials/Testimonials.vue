      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
          <div class="sm:flex-auto">
            <h1 class="can-exp-h3 mb-0 text-primary">Testimonials</h1>
            <p class="mt-2 text-sm text-gray-700">
              A list of all the testimonials
            </p>
          </div>
                  ...
                  >
                    ...
                    >
                      {{
                        ...
                        testimonial.testimonial_detail[0]
                          ? testimonial.testimonial_detail[0].name
                          : ""
                      }}
                      <dl class="font-normal lg:hidden">
                        ...
                        <dd class="mt-1 truncate text-gray-500">
                            ...
                            testimonial.testimonial_detail &&
                            testimonial.testimonial_detail[0]
                              ? testimonial.testimonial_detail[0].email
                              : ""
                          }}
                        </dd>
                        ...
                        <dd class="mt-1 truncate text-gray-500">
                          {{
                            testimonial.testimonial_detail &&
                            testimonial.testimonial_detail[0]
                              ? testimonial.testimonial_detail[0].place
                              : ""
                          }}
                        </dd>
                        <dt class="sr-only">Email</dt>
                        ...
                        <dt class="sr-only">Testimonial</dt>
                        <dd class="mt-1 truncate text-gray-500">
                          {{
                            testimonial.testimonial_detail &&
                            testimonial.testimonial_detail[0]
                              ? testimonial.testimonial_detail[0].comment
                              : ""
                          }}
                        </dd>
                    ...
                    >
                      <div>
                        <span v-if="!testimonial.showFullMessage">
                          {{
                            truncateMessage(
                              testimonial.testimonial_detail &&
                                testimonial.testimonial_detail[0]
                        ...
                        <a
                          ...
                          "
                          href="#"
                          @click.prevent="toggleMessage(testimonial)"
                          class="text-blue-600 hover:text-blue-800 text-sm"
                        >
                          {{
                            testimonial.showFullMessage
                              ? "Show Less"
                              : "Read More"
                          }}
                        </a>
                      ...
                      <div class="relative">
                        <SwitchGroup as="div" class="flex items-center">
                          <Switch
                            ...
                            :class="[
                              testimonial.status === 'active' ? 'bg-indigo-600' : 'bg-gray-200',
                          ...
                          >
                            <span
                              aria-hidden="true"
                              :class="[
                                testimonial.status === 'active' ? 'translate-x-5' : 'translate-x-0',
                ...
                <div>
                  ...
                  >
                    <a
                      ...
                      @click="fetchTestimonials(pagination.prev_page_url)"
                    >
                    ...
                    <a
                      ...
                      @click="fetchTestimonials(pagination.next_page_url)"
                    >
...
export default {
  ...
  methods: {
    ...
    fetchTestimonials(page_url) {
      this.$store.dispatch("testimonials/fetchTestimonials", { url: page_url });
    },
    updateLimit(value) {
      this.$store.commit("testimonials/setLimit", value);
      this.$store.dispatch("testimonials/fetchTestimonials");
    },
    async toggleStatus(testimonial) {
  try {
    const newStatus = testimonial.status === "active" ? "inactive" : "active";

    // Dispatch Vuex action to update the state properly
    await this.$store.dispatch("testimonials/updateTestimonialStatus", {
      id: testimonial.id,
      status: newStatus,
    });

    // API call to update status in the backend
    await axios.put(`${process.env.MIX_ADMIN_API_URL}testimonials/${testimonial.id}/status`, {
      status: newStatus,
    });
  ...
  this.$swal
        ...
        this.$swal
          ...
          .then((confirmResult) => {
            if (confirmResult.isConfirmed) {
              ...
              this.$store
                .dispatch("testimonials/deleteTestimonial", {
                  id: item.id,
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("testimonials/setSearchParam", this.quickSearch);
      this.$store.dispatch("testimonials/fetchTestimonials");
    }, 500),
    actionModal(id) {
      this.showModalId = id;
  ...
  created() {
    this.$store.commit("testimonials/setLimit", 100);
    this.$store.commit("testimonials/setSortBy", "testimonial_name");
    this.$store.commit("testimonials/setSortType", "asc");
