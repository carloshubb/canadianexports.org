...
const successStories = {
    ...
    mutations: {
        ...
        updateMessage(state, payload){
            state.form.message[`message_${payload.id}`] = payload.message;
        },
        setCompanyName(state, payload){
            state.form.company_name = payload;
        },
        updateCompanyName(state, payload){
            state.form.company_name[`company_name_${payload.id}`] = payload.company_name;
        },
        ...
        setSuccessStories(state, payload) {
            // console.log(payload);
            state.successStories = payload;
        },
        ...
        updateSuccessStoriesStatus(state, { id, status }) {
            const successStorie = state.successStories.find((t) => t.id === id);
            if (successStorie) successStorie.status = status;
          },
    },
    actions: {
        async updateSuccessStoriesStatus({ commit }, payload) {
            commit("updateSuccessStoriesStatus", payload);
          },
        addUpdateForm({ commit, state }) {
            let method = state.isFormEdit ? "put" : "post";
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}successStories/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}successStories`;
            commit("setLoading");
        ...
        fetchSuccessStories({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}successStories?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setSuccessStories", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchSuccessStorie({ commit, state }, payload) {
            let url = payload && payload.url
                ? payload.url
                : `${process.env.MIX_ADMIN_API_URL}successStories/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deleteSuccessStories({ commit }, payload) {
            commit("setLoading");
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}successStories/${payload.id}`;
            return axios
            .delete(url, {
