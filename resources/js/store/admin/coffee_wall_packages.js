...
const coffee_wall_packages = {
    ...
    mutations: {
        ...
        setRegistrationPackages(state, payload) {
            state.coffee_wall_packages = payload;
        },
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}coffee-wall-packages/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}coffee-wall-packages`;
            commit("setLoading");
        ...
        fetchRegistrationPackages({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}coffee-wall-packages?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        fetchRegistrationPackage({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}coffee-wall-packages/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deleteRegistrationPackage({ commit }, payload) {
            ...
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}coffee-wall-packages/${payload.id}`;
            return axios
            .delete(url, {
...
export default coffee_wall_packages;
