...
const contact_forms = {
    ...
    mutations: {
        setContactForms(state, payload) {
            state.contact_forms = payload;
        },
    ...
    actions: {
        fetchContactForms({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}contact-forms?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
                ...
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setContactForms", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
