...
const advertiser_forms = {
    namespaced: true,
    state: {
        advertiser_forms: null,
        loading: false,
        sortBy: "id",
        sortType: "desc",
        searchParam: null,
    ...
    mutations: {
        setAdvertiserForms(state, payload) {
            state.advertiser_forms = payload;
        },
    ...
    actions: {
        fetchAdvertiserForms({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}advertiser-forms?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
                ...
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setAdvertiserForms", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
...
export default advertiser_forms;
