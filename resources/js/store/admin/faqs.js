...
const faqs = {
    ...
    mutations: {
        setQuestion(state, payload){
            state.form.question = payload;
        },
        updateQuestion(state, payload){
            state.form.question[`question_${payload.id}`] = payload.question;
        },
        setAnswer(state, payload){
            state.form.answer = payload;
        },
        updateAnswer(state, payload){
            state.form.answer[`answer_${payload.id}`] = payload.answer;
        },
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}faqs/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}faqs`;
            commit("setLoading");
        ...
        fetchFaqs({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}faqs?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        fetchFaq({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}faqs/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deleteFaq({ commit }, payload) {
            ...
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}faqs/${payload.id}`;
            return axios
            .delete(url, {
