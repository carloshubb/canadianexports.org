...
const info_letter_forms = {
    namespaced: true,
    state: {
        info_letter_forms: null,
        loading: false,
        sortBy: "id",
        sortType: "desc",
        searchParam: null,
    ...
    mutations: {
        setInfoLetterForms(state, payload) {
            state.info_letter_forms = payload;
        },
    ...
    actions: {
        fetchInfoLetterForms({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}info-letter-forms?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
                ...
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setInfoLetterForms", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
...
export default info_letter_forms;
