...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('i2b_setting', function (Blueprint $table) {
            $table->id();
            $table->foreignId('page_id')->nullable()->constrained('pages')->onUpdate('cascade')->onDelete('cascade');
            $table->timestamps();
        });
        Schema::create('i2b_setting_detail', function (Blueprint $table) {
            $table->id();
            $table->foreignId('i2b_setting_id')->nullable()->constrained('i2b_setting')->onUpdate('cascade')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('i2b_setting_detail');
        Schema::dropIfExists('i2b_setting');
    }
};
